...
namespace TJSYXY.Communication.TCP
{
    ...
    class TCPServer
    {
        ...
        private List<TCPEndPoint> _tcpClientList=new List<TCPEndPoint>(); 
        /// <summary>
        /// 服务器列表
        /// </summary>
        ...
        public void Stop()
        {
            if (Runing)
            {
                if (_socket != null)
                {
                    ...
                    lock (objLock)
                    {
                        foreach (TCPEndPoint tcpEndPoint in _tcpClientList)
                        {
                            tcpEndPoint.TryClose();
                        }
