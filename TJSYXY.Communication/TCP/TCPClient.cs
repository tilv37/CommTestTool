...
namespace TJSYXY.Communication.TCP
{
    ...
    class TCPClient
    {
        ...
        /// 连接服务器
        /// </summary>
        /// <param name="remotEndPoint">IP节点</param>
        ...
        private void OnReceive(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                //尝试读取一条完整消息
                ZMessage msg;
                if (end.MStream.ReadMessage(out msg))
                {
                    //处理消息
        ...
        public void Send(Msg msg, byte[] data)
        {
            ...
            byte[] buffer2send = new byte[data.Length];  // 1 + 4 + data
...
//            sWriter.Write(data.Length);
            ...
            if (TCPMessageSended != null)
            {
                TCPMessageSended.BeginInvoke(buffer2send, null, null);
            }
        }
        /// <summary>
        /// 向服务器异步发送数据
