...
namespace TJSYXY.Communication.UDP
{
    ...
    class UDPClient
    {
        ...
        /// 开启UDP监听
        /// </summary>
        /// <param name="ipEndPoint">监听IP/端口号</param>
            ...
            try
            {
                ...
                if (real_recv >= 1)
                {
                    if (UDPMessageReceived != null)
                    {
                        ...
                        args.Data = new byte[real_recv];
                        Buffer.BlockCopy(_buffer4recv, 0, args.Data, 0, real_recv);
        ...
        public event UDPMessageSendedEventHandler UDPMessageSended;
        /// <summary>
        /// 同步发送数据
        /// </summary>
        ...
        public void SendTo(Msg msg, byte[] data,string remoteIP, int remotePort)
        {
            ...
            if (UDPMessageSended != null)
            {
                UDPMessageSended.BeginInvoke(buffer2send, null, null);
            }
        }
        /// <summary>
        /// 停止监听端口
